"""Parse GitHub webhook payloads into reviewbot jobs."""
...
def parse_event(
    event: str, payload: dict[str, Any], mention: str, repo: str
...
def _find_mention(body: str, mention: str) -> re.Match[str] | None:
    ...
    `@bookia-reviewer` matches `@Bookia-Reviewer` but not `@bookia-reviewer-v2`
    or `foo@bookia-reviewer` (email-like local part).
    """
    return re.search(
...
def _parse_issue_comment(
    ...
    if rest.lower().strip(" .!?") == "review":
        return ReviewJob(repo=repo, pr_number=pr_number, installation_id=installation_id)
    return DiscussJob(
        repo=repo,
        pr_number=pr_number,
