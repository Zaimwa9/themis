...
def test_parse_event__pr_opened__review_job():
    job = parse_event("pull_request", _pr_payload(), MENTION, REPO)
    assert job == ReviewJob(repo=REPO, pr_number=7, installation_id=42)


def test_parse_event__pr_ready_for_review__review_job():
    assert parse_event("pull_request", _pr_payload("ready_for_review"), MENTION, REPO) is not None


def test_parse_event__pr_draft__none():
    assert parse_event("pull_request", _pr_payload(draft=True), MENTION, REPO) is None


def test_parse_event__pr_synchronize__none():
    assert parse_event("pull_request", _pr_payload("synchronize"), MENTION, REPO) is None


def test_parse_event__wrong_repo__none():
    assert parse_event("pull_request", _pr_payload(repo="other/repo"), MENTION, REPO) is None


def test_parse_event__review_command__review_job():
    payload = _issue_comment_payload(f"{MENTION} review")
    assert parse_event("issue_comment", payload, MENTION, REPO) == ReviewJob(
        repo=REPO, pr_number=7, installation_id=42
    )


def test_parse_event__review_command_with_punctuation__review_job():
    payload = _issue_comment_payload(f"{MENTION} review!")
    assert isinstance(parse_event("issue_comment", payload, MENTION, REPO), ReviewJob)


def test_parse_event__capitalized_mention__review_job():
    payload = _issue_comment_payload("@Bookia-Reviewer review")
    assert isinstance(parse_event("issue_comment", payload, MENTION, REPO), ReviewJob)


def test_parse_event__mention_of_similar_login__none():
    payload = _issue_comment_payload("cc @bookia-reviewer-v2 hello")
    assert parse_event("issue_comment", payload, MENTION, REPO) is None


def test_parse_event__mention_as_email_local_part__none():
    payload = _issue_comment_payload("foo@bookia-reviewer review")
    assert parse_event("issue_comment", payload, MENTION, REPO) is None


def test_parse_event__mention_in_email_like_text__none():
    payload = _issue_comment_payload("email me at abc@bookia-reviewer")
    assert parse_event("issue_comment", payload, MENTION, REPO) is None


def test_parse_event__mention_wrapped_in_parens__still_recognized():
    payload = _issue_comment_payload(f"({MENTION} review)")
    job = parse_event("issue_comment", payload, MENTION, REPO)
...
def test_parse_event__review_with_extra_text__discuss_job():
    ...
    assert isinstance(parse_event("issue_comment", payload, MENTION, REPO), DiscussJob)
...
def test_parse_event__comment_without_mention__none():
    assert parse_event("issue_comment", _issue_comment_payload("lgtm"), MENTION, REPO) is None
...
def test_parse_event__review_comment_with_mention__thread_discuss_job():
    ...
    job = parse_event("pull_request_review_comment", payload, MENTION, REPO)
...
def test_parse_event__top_level_review_comment_without_mention__none():
    ...
    assert parse_event("pull_request_review_comment", payload, MENTION, REPO) is None


def test_parse_event__unknown_event__none():
    assert parse_event("push", {"repository": {"full_name": REPO}}, MENTION, REPO) is None
