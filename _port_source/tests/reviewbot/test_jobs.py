...
from reviewbot.codex import CodexError, CodexQuotaError
from reviewbot.config import ReviewBotConfig
from reviewbot.github.client import GitHubGraphQLError
from reviewbot.jobs import (
    ReviewService,
    api_changed_paths,
    git_head_sha,
    reviewbot_review_task,
)
from reviewbot.output import MAX_BODY_LEN, OUTPUT_DIR, OutputError
...
def service(gh: AsyncMock, tmp_path: Path, cleanup_calls: list[Path]) -> ReviewService:
    config = ReviewBotConfig(repo=REPO, workspace_root=tmp_path / "root")
...
async def test_review__agent_receives_configured_sandbox(service, gh):
    ...
    await service.review(REPO, 7, 42)
...
def _bot_thread() -> dict:
    return {"id": "T_1", "isResolved": False, "path": "a.py", "line": 3,
            "comments": {"nodes": [
                {"author": {"login": "bookia-reviewer"}, "body": "bug", "databaseId": 11},
...
async def test_discuss__conversation__posts_issue_comment(service, gh):
    ...
    await service.discuss(
        ...
        body="@bookia-reviewer why?", kind="conversation",
...
async def test_discuss__mentioned_in_human_thread__answers(service, gh):
    ...
    await service.discuss(
        repo=REPO, pr_number=7, installation_id=42, comment_id=22,
        body="@bookia-reviewer is this ok?", kind="thread",
...
async def test_discuss__long_thread_bot_root_outside_tail__proceeds(service):
    ...
    root = {"author": {"login": "bookia-reviewer"}, "body": "bug",
...
async def test_find_thread__matches_by_root_comment_id():
    from reviewbot.jobs import _find_thread
...
def _cancelling_service(tmp_path: Path, gh: AsyncMock) -> ReviewService:
    return ReviewService(
        config=ReviewBotConfig(repo=REPO, workspace_root=tmp_path / "nope"),
...
async def test_reviewbot_review_task__cancelled__posts_comment_and_reraises(
    ...
    monkeypatch.setattr("reviewbot.jobs._build_service", lambda: service)

    with pytest.raises(asyncio.CancelledError):
        await reviewbot_review_task({}, REPO, 7, 42)

    gh.post_issue_comment.assert_awaited_once()
    body = gh.post_issue_comment.await_args.args[2]
    assert "cancelled" in body.lower()
    assert service.config.bot.mention in body


async def test_reviewbot_review_task__real_task_cancel__posts_comment_and_reraises(
    tmp_path, gh, monkeypatch
):
    ...
    task = asyncio.ensure_future(reviewbot_review_task({}, REPO, 7, 42))
    await started.wait()
    task.cancel()
...
async def test_reviewbot_review_task__cancelled__comment_failure_still_reraises(
    tmp_path, gh, monkeypatch
):
    gh.post_issue_comment.side_effect = _http_error(401)
    service = _cancelling_service(tmp_path, gh)
